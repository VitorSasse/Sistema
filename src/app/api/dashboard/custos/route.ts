...
export async function GET(request: NextRequest) {
  ...
  const byCentro = new Map<string, { id: string | null; nome: string; total: number; count: number }>();
  ...
  for (const item of items) {
    ...
    addToMap(byCentro, item.centroCustoId ?? item.centroCustoNome, { id: item.centroCustoId, nome: item.centroCustoNome, total: 0, count: 0 }, item.subtotal);
